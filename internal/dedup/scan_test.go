...
func TestScan_VerifiedGroupingAndP0_2(t *testing.T) {
	...
	progress := &Progress{}
	groups, _, _, err := scan(context.Background(), ScanConfig{RootPath: "/", Concurrency: 4, QPS: 1000}, progress, fakeLister(tree))
	if err != nil {
...
func TestScan_NoHashStorageNeverReportsDuplicates(t *testing.T) {
	...
	progress := &Progress{}
	groups, _, _, err := scan(context.Background(), ScanConfig{RootPath: "/", Concurrency: 2}, progress, fakeLister(tree))
	if err != nil {
		t.Fatalf("scan failed: %v", err)
	}
	if len(groups) != 0 {
...
func TestScan_GCIDPreferred(t *testing.T) {
	...
	progress := &Progress{}
	groups, _, _, err := scan(context.Background(), ScanConfig{RootPath: "/"}, progress, fakeLister(tree))
	if err != nil {
		t.Fatalf("scan failed: %v", err)
	}
...
func TestScan_RootFailureIsFatal(t *testing.T) {
	...
	progress := &Progress{}
	if _, _, _, err := scan(context.Background(), ScanConfig{RootPath: "/missing"}, progress, lister); err == nil {
...
func TestScan_MaxDepth(t *testing.T) {
	...
	progress := &Progress{}
	groups, _, _, err := scan(context.Background(), ScanConfig{RootPath: "/", MaxDepth: 1, Concurrency: 2}, progress, fakeLister(tree))
	if err != nil {
		t.Fatalf("scan failed: %v", err)
	}
	if len(groups) != 0 {
